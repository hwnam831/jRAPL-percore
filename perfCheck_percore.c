...
JNIEXPORT void JNICALL Java_PerfCheckUtils_perfInit(JNIEnv * env, jclass jcls, jint numEvents, jint isSet) {
	...
	perf_event_fds = (int *) calloc(numEvents*coreNum, sizeof(int));
	if (!perf_event_fds) {
	...
	perf_event_attrs = (perf_event_attr_t *) calloc(numEvents*coreNum, sizeof(perf_event_attr_t));
	if (!perf_event_attrs) {
		errx(1, "error allocating perf_event_attrs");
	}

	for (i = 0; i < numEvents*coreNum; i++) {
...
JNIEXPORT void JNICALL Java_PerfCheckUtils_groupPerfEventsCheck(JNIEnv * env, jclass jcls, jstring name) {
	...
	for(i = 0; i < num_events*coreNum; i++) {
		int coreID = i/num_events;
		int eventID = i%num_events;
		perf_event_attr_t *attr = (perf_event_attrs + i);

		int ret = pfm_get_perf_event_encoding(event[eventID], PFM_PLM3, attr, NULL,NULL);
		if (ret != PFM_SUCCESS) {
		...
		if (eventID == 0) {
			...
			perf_event_fds[i] = perf_event_open(attr, -1, coreID, -1, 0);
		} else {
			//Open perf event for the rest members in this group.
			perf_event_fds[i] = perf_event_open(attr, -1, coreID, perf_event_fds[coreID*num_events], 0);
		}
...
JNIEXPORT void JNICALL Java_PerfCheckUtils_perfMultRead(JNIEnv * env, jclass jcls, jint core_id, jlongArray buffer) {

	long *values;
	values = (*env)->GetLongArrayElements(env, buffer, NULL);

	int ret = read(perf_event_fds[core_id*num_events], values, dataLength);
...
JNIEXPORT jint JNICALL Java_PerfCheckUtils_getCoreNum(JNIEnv * env, jclass jcls) {
	return coreNum;
}
