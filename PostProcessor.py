...
def postprocess(csvfile):
    ...
    for i in range(len(df)-100,0,-1):
        if df['B2/W:0'][i] > 2.0 or df['B2/W:1'][i] > 2.0:
            ...
            df = df.iloc[:i+1]
            break
    return df.drop(columns=['Cur power usage', 'Freq', 'Prediction', 'Gradients', 'Cur perf', 'Bips Prediction', 'New power limit', 'Cur power limit'])
    import re
...
for name in sheetnames:
    ...
    with pd.ExcelWriter(m.group(1) + '_' + m.group(3) + '.xlsx', mode=wmode) as writer:
        df.to_excel(writer, sheet_name=m.group(2))
