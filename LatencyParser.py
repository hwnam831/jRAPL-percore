...
bsizes = {
    'llama-3.1-8b' : {'high':8, 'low':2},
    'cnn-serving' : {'high':16, 'low':4},
    'stable-diffusion' : {'high':1, 'low':1},
    'vits-ljs' : {'high':2, 'low':1},
}        
def print_b2p2(folder_path):
    ...
    for csv_file in csv_files:
        ...
        mydata[configname][policy][app] += latencylist * bsizes[app][filename_parts[5]]
        print(f"{csvparts}")
    appnames.sort()
    for confname in mydata:
        with open('../'+confname + '-avg.csv', 'w') as file:
